    ...
    <button onClick={isIncompatible ? undefined : onSelect} disabled={isIncompatible}
      className={`relative text-left rounded-xl border p-4 transition-all duration-150 w-full
        ${isSelected
          ...
          : isIncompatible
            ? 'border-red-500/25 bg-red-500/5 opacity-45 cursor-not-allowed'
...
function ValidationPanel() {
  ...
  return (
    <div className="flex flex-col gap-4 h-full">
      ...
      <div className={`rounded-xl p-4 border transition-colors ${
        ...
        <div className="flex items-center gap-2.5 mb-1.5">
          ...
          <span className="text-xs font-bold tracking-wider uppercase text-white/50">
            ...
            : isBlocked       ? `${(validationResult?.critical.length ?? 0) + (validationResult?.errors.length ?? 0)} błędy`
            : hasWarnings     ? `${validationResult?.warnings.length} ostrzeżenie`
            : 'Konfiguracja OK'}
          </span>
        </div>
        <p className="text-[11px] text-white/25 leading-relaxed">
          {!selectedCount ? 'Wybierz komponenty po lewej.'
          : isBlocked     ? 'Popraw błędy aby kontynuować.'
          : hasWarnings   ? 'Konfiguracja dopuszczalna — sprawdź ostrzeżenia.'
          : `Wybrano ${selectedCount} z ${STEPS.length} komponentów.`}
        </p>
      </div>

      {/* Postęp */}
      <div className="flex flex-col gap-2">
        <p className="text-[10px] tracking-widest uppercase text-white/20">Postęp</p>
        {STEPS.map(s => (
      ...
      {hasErrors && (
        <div className="flex flex-col gap-2">
          <p className="text-[10px] tracking-widest uppercase text-orange-400/60">Błędy</p>
      ...
      {hasWarnings && (
        <div className="flex flex-col gap-2">
          <p className="text-[10px] tracking-widest uppercase text-amber-400/60">Ostrzeżenia</p>
      ...
      {isValid && !hasWarnings && selectedCount > 0 && (
        <div className="rounded-lg p-3 bg-green-500/8 border border-green-500/15">
          <p className="text-xs text-green-400">Wszystkie reguły kompatybilności spełnione ✓</p>
        </div>
      )}
      ...
      <div className="flex flex-col gap-2 pt-2 border-t border-white/5">
        ...
        {isLastStep && (
          <button onClick={handleAddToCart} disabled={isBlocked}
            ...
            {isBlocked ? 'Popraw błędy aby dodać' : 'Dodaj do koszyka →'}
          </button>
        )}
      </div>
    </div>
...
export default function Configure() {
  ...
  useEffect(() => {
    validateConfig(sel => validateConfiguration(sel) as any);
      ...
      issue.affected_groups?.forEach((g: string) => {
        if (g === step.key) {
          groups[g]?.components.forEach(c => {
            if (c.id !== selection[g as ComponentGroupKey]) {
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3">
            {group?.components.map(component => (
              <ComponentCard key={component.id} component={component}
                ...
                isIncompatible={incompatibleMap.has(component.id) && selection[step.key] !== component.id}
